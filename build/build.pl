...
my $FPGA   = ( ($target eq 'SDACCEL') || ($target eq 'AOCL') || ($target eq 'MAXELER'));
my $ocl    = ( ($target eq 'SDACCEL') || ($target eq 'AOCL') || ($target eq 'CPU') || ($target eq 'GPU'));
my $maxeler= ($target eq 'MAXELER');
...
elsif ($target eq 'MAXELER')  {$targetPlatformDirectory = '../build-maxeler';}
...
#--------------------------------------------------------------
# Overwrite STREAM_ARRAY_SIZE_DIM1 if Maxeler, 
# to make it multiple of 384 bytes
#--------------------------------------------------------------
#I am assuming 4-byte words, so the size must be multiple of 96 WORDS
#I am going to restrict each *dimension* to be 96 words
#I do a CEIL on the given DIM1 
if ($target eq 'MAXELER') {
  if($wordType eq 'INT') {
    if(($STREAM_ARRAY_SIZE_DIM1%96)!=0){
      $STREAM_ARRAY_SIZE_DIM1 = 96*(int($STREAM_ARRAY_SIZE_DIM1/96)+1); 
      print "BUILD.PL: Adjusted STREAM_ARRAY_SIZE_DIM1 to $STREAM_ARRAY_SIZE_DIM1 for Maxeler to make bytes multiple of 384\n";
    }   
  else {die "Only INT word type currently supported on MAXEKER\n";}
  }
}
...
system ("cp ../device/streamKernel.cl       $sourceFilesTarget") if($ocl);
...
# MAXELER target
#-----------------------------------------------------------
elsif ($target eq 'MAXELER') {
  system ("cp -r ../build-template/CPUCode    .");
  system ("cp -r ../build-template/EngineCode .");
  system ("cp -r ../build-template/RunRules   .");

  my $makeString;

  my $td;
  if($FLOW eq 'cpu_emu') {
    $makeString= "make runsim";
    $td = "RunRules/Simulation"; 
  }  
  else {
    $makeString = "make run";    
    $td = "RunRules/DFE"; 
...
sub printHelp {
  ...
  print "--------------------------------------------------------------------------------------\n";
  print "-tar[AOCL/SDACCEL/GPU/CPU/MAXELER]<*>: Target platform. MUST be specified.\n";
