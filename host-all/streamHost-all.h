...
#include "../common/streamEnumerations.h"
...
#include "../common/streamDerivedParameters.h"
...
static double avgtime = 0
  ...
  static double bytes = 3 * sizeof(stypeHost) * STREAM_ARRAY_SIZE;
#elif KERNELBENCH==TRIAD
  static char *label = "Triad:      ";
  static double bytes = 3 * sizeof(stypeHost) * STREAM_ARRAY_SIZE;
#else
  #error Invalid KERNELBENCH
#endif
...
void mps_display_setup(){
 // Display experimental setup
  ...
  printf("ACCESS_PATTERN          = %d [CONSTANT, CONTIGUOUS, FIXED_STRIDE] \n", ACCESS_PATTERN         );
  printf("TESTING type            = %d [RW, RO, WO]                         \n", TESTING                );
  printf("LOOPING management      = %d [API, KERNEL]                        \n", LOOPING                );
  printf("NESTING                 = %d [FLAT_LOOPING, NESTED_LOOPING]       \n", NESTING                );
  printf("STREAMSFROMTO           = %d [GMEM, HOST]                         \n", STREAMSFROMTO          );
  printf("KERNELBENCH             = %d [COPY, MUL, ADD, TRIAD]              \n", KERNELBENCH            );
  printf(HLINE);
}
...
//    #error GPU unsupported.
  ...
  const char *kernel_name = "streamKernel";
...
#if (KERNELBENCH==MUL)
   err |= clSetKernelArg(*kernel, 2, sizeof(stypeHost), (void *) scalarval);
#endif

#if (KERNELBENCH == ADD) || (KERNELBENCH == TRIAD)  
  err |= clSetKernelArg(*kernel, 2, sizeof(cl_mem), bBuffer);
  CHECK_ERRORS(err, "Error: Failed to set kernel argument 2!");
#endif
 
#if (KERNELBENCH==TRIAD)
    err |= clSetKernelArg(*kernel, 3, sizeof(stypeHost), (void *) scalarval);
#endif
